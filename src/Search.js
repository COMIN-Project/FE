...
for (let i = 0; i < 2; i++) {
  ...
  const currentReservation = roomData.find((item) => {
    const reservationDate = item.reservationDate || selectedDate; // 기본값 설정
    ...
    return (
      reservationStart.isBefore(currentEnd) && reservationEnd.isAfter(currentStart)
    );
  });
